...
public class DateSelectionPage {
	...
	private static DateSelectionPage dateSelectionPage;

	private DateSelectionPage() {
		
	}
	
	public static DateSelectionPage getDateSelectionPage() {
		
		if(dateSelectionPage==null) {
	...
	public void Changeflightsegment(int index) {
		...
		Commonfunction.getDriver().findElement(By.xpath(Segment)).click();
	}
	...
	private void OutboundselectDate(String targetMonth, String targetDate, String targetYear) {
		try {
			...
			Commonfunction.getDriver().findElement(By.xpath("//input[@id='bodycontent_RGFlightDetails_dtpFlightDate_0']")).click();
			//ChangeflightOutboundsegment.click();
			WebDriverWait wait = new WebDriverWait(Commonfunction.getDriver(), Duration.ofSeconds(10));
			//wait.until(ExpectedConditions.visibilityOfAllElements(Commonfunction.getDriver().findElement(By.xpath("//span[@class='ui-datepicker-month']"))));
			// Get the current month and year displayed in the date picker
			String currentMonth = Commonfunction.getDriver()
					.findElement(By.xpath("//span[@class='ui-datepicker-month']")).getText();
			String currentYear = Commonfunction.getDriver().findElement(By.xpath("//span[@class='ui-datepicker-year']"))
					.getText();
			...
			if (currentMonth.equalsIgnoreCase(targetMonth) && currentYear.equals(targetYear)) {
				...
				while (!currentMonth.equalsIgnoreCase(targetMonth) || !currentYear.equals(targetYear)) {
					if (shouldNavigateForward(currentMonth, currentYear, targetMonth, targetYear)) {
						...
						WebElement nextButton = Commonfunction.getDriver()
								.findElement(By.xpath("//a[@class='ui-datepicker-next ui-corner-all']"));
						wait.until(ExpectedConditions.elementToBeClickable(nextButton));
						// nextButton.click();
						((JavascriptExecutor) Commonfunction.getDriver()).executeScript("arguments[0].click();",
								nextButton);
					} else {
						// Handle case where "Previous" button might not exist (when in earliest month)
						List<WebElement> prevButtons = Commonfunction.getDriver()
						...
						if (!prevButtons.isEmpty()) {
							...
							logger.error("Previous button not found, possibly on the earliest month.");
						}
					}

					// Update the current month and year after clicking
					currentMonth = Commonfunction.getDriver()
							.findElement(By.xpath("//span[@class='ui-datepicker-month']")).getText();
					currentYear = Commonfunction.getDriver().findElement(By.xpath("//span[@class='ui-datepicker-year']"))
							.getText();
				}
			...
			logger.error("Error selecting in outbound date: " + e.getMessage());
		}
	}
	...
	private void InboundselectDate(String RetargetMonth, String RetargetDate, String RetargetYear) {
		try {
			// Open the date picker
			Commonfunction.getDriver().findElement(By.xpath("//input[@id='bodycontent_RGFlightDetails_dtpFlightDate_1']")).click();
			//Changeflightinboundsegment.click();
			...
			if (currentMonth.equalsIgnoreCase(RetargetMonth) && currentYear.equals(RetargetYear)) {
				...
				while (!currentMonth.equalsIgnoreCase(RetargetMonth) || (!currentYear.equals(RetargetYear))) {
					...
					if (shouldNavigateForward(currentMonth, currentYear, RetargetMonth, RetargetYear)) {
						...
						WebElement next = Commonfunction.getDriver()
								.findElement(By.xpath("//a[@class='ui-datepicker-next ui-corner-all']"));

						wait.until(ExpectedConditions.elementToBeClickable(next));

						((JavascriptExecutor) Commonfunction.getDriver()).executeScript("arguments[0].click();", next);

					} else {
						WebElement prev = Commonfunction.getDriver()
								.findElement(By.xpath("//a[@class='ui-datepicker-prev ui-corner-all']"));
						wait.until(ExpectedConditions.elementToBeClickable(prev));
						((JavascriptExecutor) Commonfunction.getDriver()).executeScript("arguments[0].click();", prev);
					}
